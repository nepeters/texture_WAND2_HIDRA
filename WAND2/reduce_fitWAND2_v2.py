...
def fit_WAND2(desc, rootDir, dataDir, expInfo, start, end, mode, liveplot=True, sequential=False, poleFig_type='jul', smplRot=None, ranges=None, rot_phase=None, pbaridx=None, pbarcolor='WHITE', logfile=None):
    ...
    for i,(d,ref,pi,tt) in enumerate(zip(d_all,ref_all,cnt_all,tt_all)):
        ...
        pf_dat[i+1]['data']  = np.zeros(( dataCnt, 4 ))
        pf_dat[i+1]['tt']    = tt
    ...
    if pbaridx is None:
        refine_pbar = tqdm(np.linspace(start,end,num=(end-start)+1,dtype=int),
                           desc=desc,total=dataCnt,colour=pbarcolor)
    else:
        refine_pbar = tqdm(np.linspace(start,end,num=(end-start)+1,dtype=int),
                           desc=desc,position=pbaridx,total=dataCnt,colour=pbarcolor)       
    ...
    for run in refine_pbar:
        ...
        chi     = runInfo['Chi'].to_numpy()[0] ## 90 - chi for original projection
        ...
        for dd,eta in inner_iter:
            ...
            for i in tt_pres_num:
                ...
                # set amplitude
                I_meas = meas_int[np.argmin( np.abs( tt_ran - pf_dat[i]['tt'] ) )]
                ...
                params['p{}_sigma'.format(i)].set(value=sig_guess)
            ...
            for i in tt_pres_num:
                ...
                # get projection (q)
                q = rotate_project_q(tt/2, omega, chi, phi, eta)
                # store it
    ...
    for run in refine_pbar:
        ...
        for dd,eta in inner_iter:
            ...
            for i in tt_pres_num:
                ...
                # calc integ. int
                II = np.trapz(comps['p{}_'.format(i)],
                                x = tt_ran,
                                dx = tt_ran[1]-tt_ran[0])
