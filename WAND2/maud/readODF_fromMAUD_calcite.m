...
main_dir     = 'C:\Users\np7ut\Dropbox\ORNL\Texture\';

parfile_dirs = ["WAND2\maud\Calcite_texred",...
                "VULCAN\June2020\7Banks"];

parfiles     = ["\HB2C_MAUD_Calcite_EWIMV10.par",...
                "\Calcite_Vulcan_EWIMV10_redo.par"];
...
for i=1:length(parfiles)
    ...
    export_dir = strcat(main_dir, parfile_dirs(i),"\exports");
    parfile_loc = strcat(main_dir, parfile_dirs(i), parfiles(i));
    ...
    %         cellVolume = deg2rad(odf_in.res(pi)) * deg2rad(odf_in.res(pi)) * delta_Phi;
        ...
        odf_out = strcat(export_dir,"\",name,"_mtex.odf");
        ...
        saveFigure(char(strcat(export_dir,"\",name,"_pfs.jpg")))
        close
