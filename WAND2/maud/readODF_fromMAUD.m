...
%% Load PF for inversion

% % crystal symmetry
% odf_in.cs{pi} = crystalSymmetry('m-3m', [2.9 2.9 2.9]);
% odf_in.cs{pi} = crystalSymmetry('m-3m', [3.6 3.6 3.6]);
% 
% % specimen symmetry
...
% 
% % path to files
% pname = 'C:\Users\np7ut\Dropbox\ORNL\Texture\WAND2\maud\samples_5deg\Steel_2';
% 
% % which files to be imported
% fname_bcc = [pname '\HB2C_MAUD_Steel_2_Fe(BCC).pf'];
% fname_fcc = [pname '\HB2C_MAUD_Steel_2_Fe(FCC).pf'];
% 
% % Specify Miller Indice
% h_bcc = { ...
%   Miller(1,1,0,odf_in.cs{pi}),...
...
% pf_bcc = PoleFigure.load(fname_bcc,h_bcc,odf_in.cs{pi},SS,'interface','beartex');
% pf_bcc.intensities = pf_bcc.intensities ./ 100;
% 
% pf_fcc = PoleFigure.load(fname_fcc,h_fcc,odf_in.cs{pi},SS,'interface','beartex');
% pf_fcc.intensities = pf_fcc.intensities ./ 100;
% 
% reodf_bcc = calcODF(pf_bcc,'silent');
% reodf_fcc = calcODF(pf_fcc,'silent');
% 
% figure
% plot(pf_bcc{1:3},'contourf','colorrange',[0 6])
% mtexColorbar('title','direct PF plot')
% 
% figure
% plotPDF(odf{2}{1},h_bcc(1:3),'contourf','colorrange',[0 6])
% mtexColorbar('title','from ODF import')
% 
% figure
% plotPDF(reodf_bcc,h_bcc(1:3),'contourf','colorrange',[0 6])
% mtexColorbar('title','reinvert from PF')

%%

% for i=2:10
%     
    ti_bcc = textureindex(odf{i}{1});
    ti_fcc = textureindex(odf{i}{2});
    
    name_odf = ['Steel_' int2str(i)];
    
    fprintf(' %s | Texture index | FCC: %f6 | BCC: %f6\n',name_odf,ti_fcc,ti_bcc)
%     
% end
