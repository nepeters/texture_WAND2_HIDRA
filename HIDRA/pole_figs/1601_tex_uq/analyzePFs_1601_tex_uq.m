...
% created from reduce_fit python script @ 2022-07-20 15:38:38.327032
...
SS_1 = specimenSymmetry('1');
CS_2 = crystalSymmetry('m-3m', [1 1 1]); % lattice-type:F
SS_2 = specimenSymmetry('1');

% plotting convention
setMTEXpref('xAxisDirection','north');
setMTEXpref('zAxisDirection','outOfPlane');

rot1 = rotation('axis',zvector,'angle',93*degree);
...
% path to files
fname_BCC = {...
  '1601_tex_uq_BCC_220.pf',...
  '1601_tex_uq_BCC_200.pf',...
  };

fname_FCC = {...
  '1601_tex_uq_FCC_222.pf',...
  '1601_tex_uq_FCC_311.pf',...
  '1601_tex_uq_FCC_220.pf',...
  };
...
%         npf_FCC(npf_FCC.isOutlier) = [];
        ...
        inv_g       = inv(grains);
        THill_HEM   = calcTensor(grains, c_ijkl, 'Hill');
        diff_fibre  = fibre(Miller(3,1,1,CS_2), vector3d.Y);
        ind         = angle(diff_fibre, grains) < 10*degree;
        diff_grains = grains(ind); 
        THill_diff  = calcTensor(diff_grains, s_ijkl);
        THill = THill_diff+inv(THill_HEM);
